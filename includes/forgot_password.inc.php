...
if (isset($_POST['reset'])) {
  ...
  if ($rowUserAdmin['numrows'] > 0) {
    //generate code
    $setUserAdmin = '123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $codeUserAdmin = substr(str_shuffle($setUserAdmin), 0, 15);

    try {
      $stmtUserAdmin = $conn->prepare("UPDATE users SET reset_code=:code WHERE id=:id");
      $stmtUserAdmin->execute([
        'code' => $codeUserAdmin,
        'id' => $rowUserAdmin['id']
      ]);
      $url = 'http://localhost/palengke_go/recover_password.php?code=' . $codeUserAdmin . '&user=' . $rowUserAdmin['id'];
      $message = "
				<html>
          <head>
                          ...
                          <tr>
                            <td>
                              <center>
                                ...
                                <p><h2>Password Reset Request</h2></p>
                              </center>
                            </td>
                          </tr>
                        </table>
      ...
      try {
        ...
        $mailUserAdmin->Subject = 'Password Reset from PalengkeGo! Mariveles Public Market App';
    ...
    if ($rowStallholder['numrows1'] > 0) {
      //generate code
      $setStallholder = '123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
      $codeStallholder = substr(str_shuffle($setStallholder), 0, 15);

      try {
        $stmtStallholder = $conn->prepare("UPDATE stall_holders SET reset_code=:code WHERE id=:id");
        $stmtStallholder->execute([
          'code' => $codeStallholder,
          'id' => $rowStallholder['id']
        ]);
        $url = 'http://localhost/palengke_go/recover_password.php?code=' . $codeStallholder . '&user=' . $rowStallholder['id'];
        $message = "
          <html>
            <head>
              <meta name='viewport' content='width=device-width' />
        ...
        try {
          ...
          $mailStallholder->Subject = 'Password Reset from PalengkeGo! Mariveles Public Market App';
