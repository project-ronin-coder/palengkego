...
if (!isset($_GET['code']) or !isset($_GET['user'])) {
  ...
  $_SESSION['message'] = 'Code to reactivate account not found.';
  $output .= '
      <!-- LOGO -->
      ...
      <div class="card">
        <div class="card-body login-card-body ">
          <h3 class="login-box-msg-header">Account Reactivation Error!</h3>
  ...
  if ($rowUserAdmin['numrows'] > 0) {
    if ($rowUserAdmin['act']) {
      ...
      try {
        $stmtUpdateUserAdmin = $conn->prepare("UPDATE users SET act=:act WHERE id=:id");
        $stmtUpdateUserAdmin->execute(['act' => 1, 'id' => $rowUserAdmin['id']]);
        $full_name = $rowUserAdmin['first_name'] . ' ' . $rowUserAdmin['last_name'];
        $_SESSION['success'] = 'Success!';
        $_SESSION['message'] = 'Account Reactivated!';
        $output .= '
                <!-- LOGIN -->
                <div class="login-logo">
                  <img src="assets/img/PalengkeGO_Main_Logo.png" alt="picture logo of PalengkeGO" class="logo">
                ...
                <div class="card">
                  <div class="card-body login-card-body">
                    <h3 class="login-box-msg-header">Account Reactivated!</h3>
    ...
    if ($rowStallholder['act']) {
      ...
      try {
        $stmtUpdateStallholder = $conn->prepare("UPDATE stall_holders SET act=:act WHERE id=:id");
        $stmtUpdateStallholder->execute(['act' => 1, 'id' => $rowStallholder['id']]);
    ...
    $_SESSION['error'] = 'Error!';
    $_SESSION['message'] = "Cannot reactivate account. Wrong code.";
    $output .= '
        <!-- LOGO -->
        <div class="login-logo">
          <img src="assets/img/PalengkeGO_Main_Logo.png" alt="picture logo of PalengkeGO" class="logo">
        ...
        <div class="card">
          <div class="card-body login-card-body">
            <h3 class="login-box-msg-header text-danger">Account Reactivation Error!</h3>
...
<head>
  ...
  <title>Palengke Go | Reactivate Account</title>
